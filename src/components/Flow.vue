...
if (res.sourceSystemArray && res.sceneArray && res.targetSystemArray && res.linksArray) {
  let obj = {
    nodes: [
      res.sourceSystemArray.map((node, i) => ({
        id: `0-${i}`,
        ...node
      })),
      res.sceneArray.map((node, i) => ({
        id: `1-${i}`,
        ...node
      })),
      res.targetSystemArray.map((node, i) => ({
        id: `2-${i}`,
        ...node
      }))
...
console.log('🚀 ~ nodes.value=flowData.value.nodes.flatMap ~ nodes.value:===》\n', nodes.value)
...
<template>
  <VueFlow :nodes="nodes" :edges="edges">
    ...
    <template #node-set="props">
      <SetNode :id="props.id" :data="props.data" />
    </template>
